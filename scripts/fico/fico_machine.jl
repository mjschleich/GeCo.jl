using CSV, Statistics, DataFrames, MLJModelInterface, MLJ, MLJBase


#MLJModelInterface.@mlj_model mutable struct FICO_CLASSIFIER <: MLJModelInterface.Deterministic
#    a::Float64 = 0.5::(_ > 0)
#end

mutable struct FICO_CLASSIFIER <: MLJModelInterface.Deterministic
    a::Float64
end

function FICO_CLASSIFIER(; a=0.5)
    model   = FICO_CLASSIFIER(a)
    message = MLJModelInterface.clean!(model)
    isempty(message) || @warn message
    return model
end
function MLJModelInterface.clean!(m::FICO_CLASSIFIER)
    warning = ""
    if m.a <= 0
...
function MLJModelInterface.fit(model::FICO_CLASSIFIER, verbosity, X, y)
    fitresult = nothing
    cache = nothing
    report = nothing
...
function MLJModelInterface.predict(model::FICO_CLASSIFIER, fitresult, Xnew)
    outcomes = Array{Float32}(undef, size(Xnew)[1])
    for i in 1:size(Xnew)[1]
        outcomes[i] = 1 - risk_model(Xnew[i, :])
    end
    a = categorical([0,1])
    return [MLJBase.UnivariateFinite(a, outcome, augment=true) for outcome in outcomes]
end
...
input = "data/fico/heloc_dataset_v1.csv"
X = CSV.File(input) |> DataFrame
y = X[!, 1]
X = select!(X, Not(1))
...
coerce!(X,
    :ExternalRiskEstimate => Count,
    :MSinceOldestTradeOpen => Count,
    :MSinceMostRecentTradeOpen => Count,
    :AverageMInFile => Count,
    :NumSatisfactoryTrades => Count,
    :NumTrades60Ever2DerogPubRec => Count,
    :NumTrades90Ever2DerogPubRec => Count,
    :PercentTradesNeverDelq => Count,
    :MSinceMostRecentDelq => Count,
    :MaxDelq2PublicRecLast12M => Count,
    :MaxDelqEver => Count,
    :NumTotalTrades => Count,
    :NumTradesOpeninLast12M => Count,
    :PercentInstallTrades => Count,
    :MSinceMostRecentInqexcl7days => Count,
    :NumInqLast6M => Count,
    :NumInqLast6Mexcl7days => Count,
    :NetFractionRevolvingBurden => Count,
    :NetFractionInstallBurden => Count,
    :NumRevolvingTradesWBalance => Count,
    :NumInstallTradesWBalance => Count
)

orig_instance = X[6, :]

fico = FICO_CLASSIFIER(0.5)
classifier = machine(fico, X, y)
fit!(classifier)
p = initPLAF()