...
# occ_group = Tuple(s for s in propertynames(X) if contains(string(s), "Occupation"))
...
# ms_group = Tuple(s for s in propertynames(X) if contains(string(s), "MaritalStatus"))
...
# workclass_group = Tuple(s for s in propertynames(X) if contains(string(s), "WorkClass"))
